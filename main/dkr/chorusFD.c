...
void ChorusDelay_init(monochorus_t *del, float delay, float dfb, float dfw, float dmix)
{
	...
	if(del->dline==NULL)
	{
		printf("ChorusDelay_init(): allocating %d bytes\n", (DEPTH+2) * sizeof(float));
		del->dline = (float*)malloc((DEPTH+2) * sizeof(float));
		if(del->dline==NULL)
		{
			printf("ChorusDelay_init(): dline buffer could not be allocated\n");
			while(1) { error_blink(5,60,0,0,5,30); /*RGB:count,delay*/ };
		}
		else
		{
			printf("ChorusDelay_init(): dline buffer allocated at %x\n", (unsigned int)del->dline);
		}
	}
	else
	{
		printf("ChorusDelay_init(): dline buffer already allocated at %x\n", (unsigned int)del->dline);
	}
}
...
/*---------------------------------------------------------------------------------------------*/
void Chorus_deinit(void)
{
	printf("Chorus_deinit()\n");
	if(delL.dline!=NULL)
	{
		free(delL.dline);
		delL.dline = NULL;
		printf("Chorus_deinit(): delL buffer freed\n");
	}
	if(delR.dline!=NULL)
	{
		free(delR.dline);
		delR.dline = NULL;
		printf("Chorus_deinit(): delR buffer freed\n");
	}
}
/*---------------------------------------------------------------------------------------------*/
